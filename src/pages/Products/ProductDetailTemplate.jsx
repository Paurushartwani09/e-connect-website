...
/* feature icon rotation — picks from a small set */
const featureIconSet = [FiZap, FiCheckCircle, FiPackage, FiArrowRight, FiMail, FiCloud]
...
export default function ProductDetailTemplate({ product: p }) {
  if (!p) return null
        ...
        <div className={styles.container}>
          <div className={styles.overviewGrid}>
            <motion.div className={styles.overviewText} {...fadeUp(0)}>
              ...
              {p.fullDesc.split('. ').reduce((acc, sentence, i, arr) => {
                if (i % 2 === 0) {
                  const pair = arr.slice(i, i + 2).join('. ')
                  if (i % 2 === 0) acc.push(<p key={i}>{pair + (arr[i + 1] ? '' : '')}</p>)
                }
                return acc
              }, [])}
            </motion.div>
        ...
        <div className={styles.container}>
          <motion.div className={styles.sectionHeader} {...fadeUp(0)}>
            ...
            <h2 className={styles.sectionTitle}>
              Key <span className={styles.accent}>Modules</span>
            </h2>
            <p className={styles.sectionSub}>
              Purpose-built components that work together as one cohesive system
            </p>
          </motion.div>
      ...
      <section className={styles.featuresSection}>
        <div className={styles.container}>
          <motion.div className={styles.sectionHeader} {...fadeUp(0)}>
            ...
            <h2 className={styles.sectionTitle}>
              Key <span className={styles.accent}>Features</span>
            </h2>
            <p className={styles.sectionSub}>
              Built-in capabilities that deliver measurable business value from day one
            </p>
          </motion.div>
      ...
      <section className={styles.useCasesSection}>
        <div className={styles.container}>
          <motion.div className={styles.sectionHeader} {...fadeUp(0)}>
            ...
            <h2 className={styles.sectionTitle}>
              Use <span className={styles.accent}>Cases</span>
            </h2>
            <p className={styles.sectionSub}>
              How organisations across India are transforming operations with this product
            </p>
          </motion.div>
