...
import { Users, Building2, LayoutGrid, CalendarDays, CreditCard, Landmark, Lock, LucideIcon } from 'lucide-react'
import styles from './Stats.module.css'

interface StatItem { Icon: LucideIcon; value: number; suffix: string; label: string; desc: string; color: string }
interface HighlightItem { Icon: LucideIcon; text: string }
...
interface CountUpProps { target: number; suffix: string; active: boolean }
    ...
    <section className={styles.section} ref={ref}>
      ...
      <div className={styles.container}>
        <motion.div className={styles.header}
          ...
          <p className={styles.desc}>Hundreds of E-Governance projects where millions of citizens use our applications.</p>
        </motion.div>
          ...
          {stats.map(({ Icon, value, suffix, label, desc, color }, i) => (
            <motion.div key={label} className={styles.card}
              ...
              <div className={styles.cardIconRing}>
                <div className={styles.cardIconInner}><Icon size={22} strokeWidth={1.5} /></div>
              </div>
              <div className={styles.cardNum}><CountUp target={value} suffix={suffix} active={inView} /></div>
