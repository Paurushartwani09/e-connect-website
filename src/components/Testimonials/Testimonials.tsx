...
export default function Testimonials() {
  ...
  return (
    <section className={styles.testimonials} ref={ref}>
      <div className={styles.bgQuote} aria-hidden="true">"</div>
      <div className={styles.container}>
        <motion.div className={styles.header} variants={staggerContainer} initial="hidden" animate={inView ? 'visible' : 'hidden'}>
          <motion.span className={styles.sectionTag} variants={fadeUp}>Our Team Says</motion.span>
          <motion.h2 className={styles.title} variants={fadeUp}>
            Voices from<span className={styles.titleAccent}> E-Connect</span>
          </motion.h2>
        </motion.div>

        <motion.div className={styles.testimonialWrap}
          initial={{ opacity: 0, y: 40 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          transition={{ delay: 0.3, duration: 0.7 }}>
          <div className={styles.quoteIcon}>"</div>
          <AnimatePresence mode="wait">
            <motion.div key={active} className={styles.testimonialContent}
