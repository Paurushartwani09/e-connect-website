...
function AdvCard({ adv, index, inView }: AdvCardProps) {
  const { Icon, color, num, title, desc } = adv
  return (
    <motion.div className={styles.card} style={{ '--c': color } as React.CSSProperties}
...
function TrustBanner({ inView }: TrustBannerProps) {
  return (
